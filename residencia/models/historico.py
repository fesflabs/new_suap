...
class SituacaoMatricula(LogResidenciaModel):
    MATRICULADO = 1
    TRANCADO = 2
    JUBILADO = 3
    ...
    def get_absolute_url(self):
        return '/residencia/visualizar/edu/situacaomatricula/{}/'.format(self.pk)

class SituacaoMatriculaPeriodo(LogResidenciaModel):
    EM_ABERTO = 1
    MATRICULADO = 2
    TRANCADA = 3
    CANCELADA = 4
    ...
    def get_absolute_url(self):
        return '/residencia/visualizar/residencia/situacaomatriculaperiodo/{}/'.format(self.pk)


class MatriculaPeriodo(LogResidenciaModel):
    SEARCH_FIELDS = ['residente__pessoa_fisica__nome_registro', 'residente__pessoa_fisica__nome_social', 'residente__matricula']

    residente = models.ForeignKeyPlus('residencia.Residente', related_name='matriculas_periodos_residente_residencia_set',)
    ano_letivo = models.ForeignKeyPlus('comum.Ano', verbose_name='Ano Letivo', related_name='matriculas_periodos_ano_letivo_residencia_set', on_delete=models.CASCADE )
    ...
    turma = models.ForeignKeyPlus('residencia.Turma', related_name='matriculas_periodos_turma_residencia_set', null=True, on_delete=models.SET_NULL)
    situacao = models.ForeignKeyPlus('residencia.SituacaoMatriculaPeriodo', verbose_name='Situação', related_name='matriculas_periodos_situacao_residencia_set', on_delete=models.CASCADE)
    ...
    #     from residencia.models import HorarioAulaDiario
    #
    ...
    #     from residencia.models.diarios import ConfiguracaoAvaliacao
    #     series = []
    #     for matricula_diario in self.matriculas_diarios_matricula_periodo_residencia_set.all():
    #         nota_residente = 0
    #         media_turma = 0
    #         adicionar = True
    #         if etapa > 5:
    ...
    #         if adicionar:
    #             if etapa == 6:
    #                 nota_residente = matricula_diario.get_media_disciplina()
    #                 media_turma = matricula_diario.diario.get_media_medias()
    #             elif etapa == 7:
    #                 nota_residente = matricula_diario.get_media_final_disciplina()
    #                 media_turma = matricula_diario.diario.get_media_medias_finais()
    #             else:
    #                 nota_residente = matricula_diario.get_nota_by_etapa(etapa)
    #                 media_turma = matricula_diario.diario.get_media_notas_by_etapa(etapa)
    #
    #         series.append([matricula_diario.diario.componente_curricular.componente.descricao, nota_residente, media_turma])
    #     return series
    #
    # def adicionar_registro_historico(self, matricula_periodo):
    #
    #     for matricula_diario in self.matriculas_diarios_matricula_periodo_residencia_set.filter(situacao__in=[MatriculaDiario.SITUACAO_APROVADO]):
    ...
    #     from residencia.models import CertificacaoConhecimento
    #
    ...
    #     if self.residente.matriculaperiodo_set.filter(id__gt=self.pk).count() == 1:
    #         if somente_em_aberto:
    #             qs = self.residente.matriculaperiodo_set.filter(id__gt=self.pk, situacao=SituacaoMatriculaPeriodo.EM_ABERTO)
    #         else:
    #             qs = self.residente.matriculaperiodo_set.filter(id__gt=self.pk)
    ...
    #         return True
        if self.matriculas_diarios_diario_residencia_set.exists():
            return True
        if self.matriculas_diarios_matricula_periodo_residencia_set.exists():
            return True
    ...
    #     from residencia.models import CursoCampus
    #
    #     if self.residente.curso_campus.periodicidade == CursoCampus.PERIODICIDADE_ANUAL:
    ...
    #             periodo_letivo = 2
    #
    #     qs = MatriculaPeriodo.objects.filter(residente=self.residente, ano_letivo=ano_letivo, periodo_letivo=periodo_letivo)
    #     if qs.exists():
    #         matricula_periodo = qs[0]
    #     else:
    #         matricula_periodo = MatriculaPeriodo()
    #         matricula_periodo.residente = self.residente
    ...
    #     from residencia.models import EstruturaCurso
    #
    ...
    #         if (
    #             self.residente.matriz.estrutura.tipo_avaliacao == EstruturaCurso.TIPO_AVALIACAO_CREDITO
    #             or self.matriculas_diarios_matricula_periodo_residencia_set.filter(situacao=MatriculaDiario.SITUACAO_APROVADO).exists()
    #         ):
    #             periodo_matriz = self.periodo_matriz + 1
    #         if periodo_matriz > self.residente.matriz.qtd_periodos_letivos and self.residente.matriz.estrutura.tipo_avaliacao in [
    #             EstruturaCurso.TIPO_AVALIACAO_SERIADO,
    #             EstruturaCurso.TIPO_AVALIACAO_MODULAR,
    #         ]:
    #             periodo_matriz = self.residente.matriz.qtd_periodos_letivos
    #     else:
    #         periodo_matriz = self.periodo_matriz
    ...
    #     return self.matriculas_diarios_matricula_periodo_residencia_set.filter(situacao__in=ids_situacoes, matricula_periodo=self).count()
    #
    # def is_cancelado(self):
    #     from residencia.models import SituacaoMatricula
    #
    #     return self.residente.situacao.pk in (
    ...
    #     return self.residente.is_matriculado()
    #
    ...
    #     from residencia.models import SituacaoMatricula
    #
    #     return self.residente.situacao.pk == SituacaoMatricula.MATRICULADO or self.residente.situacao.pk == SituacaoMatricula.CONCLUDENTE
    #
    # def is_aberto(self):
    ...
    def pode_remover_da_turma(self, user):
        ...
        if user.has_perm('residencia.delete_matriculadiario_cancelada_trancada'):
        ...
        result = self.residente.situacao.pk in SITUACOES_MATRICULA_PARA_EXCLUSAO and self.situacao.pk in SITUACOES_MATRICULA_PERIODO_PARA_EXCLUSAO
        if result:
            result = user.has_group(['Secretário(a) Residência','Coordenadores(as) Residência'])or user.is_superuser
      #TODO
            # result = user.has_group(
            #     'Administrador Acadêmico') or user.is_superuser or not self.possui_nota_lancada_na_turma() and not self.possui_falta_lancada_na_turma()
        return result
    #
    # def pode_matricular_diario(self):
    #     return (self.residente.iniciou_suap() or self.residente.integralizou_suap()) and self.situacao.pk in [SituacaoMatriculaPeriodo.MATRICULADO, SituacaoMatriculaPeriodo.EM_ABERTO]
    #
    ...
    #     from residencia.models import SituacaoMatricula
    #
    #     situacao_matricula = self.residente.situacao.ativo or self.residente.situacao.pk in [SituacaoMatricula.TRANCADO, SituacaoMatricula.TRANCADO_VOLUNTARIAMENTE]
    ...
    #     from residencia.models import EstruturaCurso
    #
    #     if self.residente.matriz.estrutura.tipo_avaliacao == EstruturaCurso.TIPO_AVALIACAO_MODULAR and procedimento.tipo in [
    ...
    #         if not self.matriculas_diarios_matricula_periodo_residencia_set.filter(diario__turma__pertence_ao_plano_retomada=True).exists():
    #             raise ValidationError('Não é possível realizar o trancamento de um residente do regime modular.')
    #
    #     mps = MatriculaPeriodo.objects.filter(residente=self.residente, situacao__id__in=[SituacaoMatriculaPeriodo.EM_ABERTO, SituacaoMatriculaPeriodo.MATRICULADO]).exclude(pk=self.pk)
    #     if mps.exists():
    #         raise ValidationError('É necessário fechar os períodos anteriores antes de realizar o procedimento')
    #
    #     procedimento.save()
    #     self.residente.situacao = situacao_matricula
    #     self.residente.save()
    #     self.situacao = situacao_matricula_periodo
    #     self.save()
    #     self.matriculas_diarios_matricula_periodo_residencia_set.filter(situacao=MatriculaDiario.SITUACAO_CURSANDO).update(situacao=situacao_matricula_diario)
    #
    #     if self.residente.candidato_vaga_id:
    ...
    #             self.residente.candidato_vaga.situacao = CandidatoVaga.MATRICULA_CANCELADA
    ...
    #             self.residente.candidato_vaga.situacao = CandidatoVaga.TRANSFERIDO
    #         if procedimento.tipo == ProcedimentoMatricula.EVASAO:
    #             self.residente.candidato_vaga.situacao = CandidatoVaga.EVADIDO
    #         self.residente.candidato_vaga.save()
    #
    # @transaction.atomic
    # def reintegrar_residente(self, procedimento, ano_letivo, periodo_letivo):
    #     residente = self.residente
    #     residente.situacao = residente.get_ultimo_procedimento_matricula().situacao_matricula_anterior
    #     residente.save()
    #
    #     matricula_periodo = MatriculaPeriodo()
    ...
    #     matricula_periodo.ano_letivo = ano_letivo
    #     matricula_periodo.periodo_letivo = periodo_letivo
    #     matricula_periodo.periodo_matriz = residente.periodo_atual or 1
    #     matricula_periodo.turma = None
    #     matricula_periodo.situacao = SituacaoMatriculaPeriodo.objects.get(pk=SituacaoMatriculaPeriodo.EM_ABERTO)
    ...
    def get_matriculas_diario(self):
        qs_matriculas_diario = self.matriculas_diarios_matricula_periodo_residencia_set.all().order_by('diario__componente_curricular__componente__descricao_historico')
    ...
    def get_matriculas_diario_ativas(self):
        ...
        return self.matriculas_diarios_matricula_periodo_residencia_set.exclude(situacao__in=ids)

    def possui_nota_lancada_na_turma(self):
        return self.possui_nota_lancada(self.turma)

    def possui_nota_lancada(self, turma=None):
        qs = self.matriculas_diarios_matricula_periodo_residencia_set.exclude(situacao=MatriculaDiario.SITUACAO_TRANSFERIDO)
        if turma:
    ...
    #     from residencia.models.diarios import Falta
    #
    ...
    def get_media_na_etapa(self, etapa):
        ...
        return self.matriculas_diarios_matricula_periodo_residencia_set.all().aggregate(Avg(avg))['{}__avg'.format(avg)]
    ...
    #     elif self.matriculas_diarios_matricula_periodo_residencia_set.exists():
    #         return self.matriculas_diarios_matricula_periodo_residencia_set.all()[0].diario.turma.get_calendario_academico()
    #     return None
    #
    ...
    #     from residencia.models.diarios import Aula
    #
    #     situacoes_inativas = [MatriculaDiario.SITUACAO_CANCELADO, MatriculaDiario.SITUACAO_DISPENSADO, MatriculaDiario.SITUACAO_TRANCADO, MatriculaDiario.SITUACAO_TRANSFERIDO]
    #     qs = Aula.objects.filter(
    #         professor_diario__diario__matriculadiario__in=self.matriculas_diarios_matricula_periodo_residencia_set.exclude(situacao__in=situacoes_inativas).values_list('pk', flat=True),
    ...
    #     return self.get_percentual_carga_horaria_frequentada() >= self.residente.matriz.estrutura.percentual_frequencia
    #
    ...
    #     procedimento.situacao_matricula_anterior = self.residente.situacao
    ...
    #     percentual_frequencia_minimo = self.residente.matriz.estrutura.percentual_frequencia
    #     # plano de retomada de aulas em virtude da pandemia (COVID19)
    #     if self.matriculas_diarios_matricula_periodo_residencia_set.filter(diario__turma__pertence_ao_plano_retomada=True).exists():
    #         percentual_frequencia_minimo = 0
    #     return percentual_frequencia_minimo
    #
    # def fechar_periodo_letivo(self, forcar_fechamento=False):
    #     from residencia.models import HistoricoSituacaoMatriculaPeriodo, SituacaoMatricula, EstruturaCurso
    #
    #     if self.pode_fechar_periodo_letivo() or forcar_fechamento:
    #         for matricula_diario in self.matriculas_diarios_matricula_periodo_residencia_set.all():
    #             matricula_diario.fechar_periodo_letivo(forcar_fechamento)
    #
    #         estrutura = self.residente.matriz.estrutura
    #         # FIC
    ...
    #
    #         # Caso o residente não tenha atindigo o percentual mínino de frequência no período, a situação dele no período
    ...
    #
    #         self.save()
    #         self.residente.atualizar_situacao('Fechamento de Período')
    #
    #         if not self.residente.curso_campus.is_fic() and self.residente.situacao.pk == SituacaoMatricula.MATRICULADO:
    #             matricula_periodo = self.instanciar_proxima_matricula_periodo()
    #             matricula_periodo.save()
    #
    #             self.residente.atualizar_periodo_referencia()
    #             self.residente.save()
    #
    #         if self.residente.situacao.pk == SituacaoMatricula.MATRICULADO and self.residente.get_ultima_matricula_periodo().situacao.pk == SituacaoMatriculaPeriodo.EM_ABERTO:
    #             motivo = self.residente.deve_ser_jubilado()
    #             if motivo:
    #                 self.residente.jubilar(motivo, True)
    #
    # def abrir_periodo_letivo(self):
    #     from residencia.models import SituacaoMatricula, HistoricoSituacaoMatriculaPeriodo
    #
    #     ids = []
    ...
    #
    #     self.residente.situacao = SituacaoMatricula.objects.get(id=SituacaoMatricula.MATRICULADO)
    #     self.residente.save()
    #
    #     self.residente.atualizar_situacao('Reabertura de Período')
    #
    # def __str__(self):
    #     return '{} - {}º Período ({}.{})'.format(str(self.residente), self.periodo_matriz, self.ano_letivo.ano, self.periodo_letivo)
    #
    ...
    #     from residencia.models import HorarioCampus
    #     from residencia.models.diarios import Diario
    #
    ...
    #     from residencia.models import HorarioAulaDiario, HorarioCampus, Turno
    #     from residencia.models.diarios import Diario
    #
    ...
    #     from residencia.models import HorarioAulaDiario, HorarioCampus, Turno, Diario, HorarioAtividadeExtra
    #
    #     dias_semana = HorarioAulaDiario.DIA_SEMANA_CHOICES
    #     horarios_campus_ids = Diario.objects.filter(matriculadiario__matricula_periodo=self).values_list('horario_campus__id', flat=True)
    #     turnos_ids = HorarioCampus.objects.filter(id__in=horarios_campus_ids).values_list('horarioaula__turno__id', flat=True)
    ...
    #         situacoes_inativas = [MatriculaDiario.SITUACAO_CANCELADO, MatriculaDiario.SITUACAO_DISPENSADO, MatriculaDiario.SITUACAO_TRANCADO, MatriculaDiario.SITUACAO_TRANSFERIDO]
    #         qs = qs.filter(professor_diario__diario__matriculadiario__in=self.matriculas_diarios_diario_residencia_set.exclude(situacao__in=situacoes_inativas).values_list('pk', flat=True))
    #     soma = 0
    #     pks = []
    ...
    #     from residencia.models import SituacaoMatricula
    #
    #     if self.residente.situacao.pk in [SituacaoMatricula.AGUARDANDO_COLACAO_DE_GRAU, SituacaoMatricula.EGRESSO, SituacaoMatricula.FORMADO, SituacaoMatricula.CONCLUIDO]:
    #         return True
    #     if self.situacao.pk in [
    #         SituacaoMatriculaPeriodo.TRANCADA,
    #         SituacaoMatriculaPeriodo.AFASTADO,
    ...
    #         return self.residente.situacao.pk in [SituacaoMatricula.CONCLUIDO, SituacaoMatricula.FORMADO]
    #     if self.situacao.pk in [SituacaoMatriculaPeriodo.TRANSF_TURNO, SituacaoMatriculaPeriodo.TRANSF_CURSO]:
    #         return 'Verificar no Q-Acadêmico'
    #     return None
    #
    ...
    #     if self.situacao.pk in [SituacaoMatriculaPeriodo.APROVADO, SituacaoMatriculaPeriodo.ESTAGIO_MONOGRAFIA, SituacaoMatriculaPeriodo.PERIODO_FECHADO]:
    #         return (self.residente.situacao.pk in [SituacaoMatricula.CONCLUIDO, SituacaoMatricula.FORMADO]) and 'Formado' or 'Cursando'
    #     if self.situacao.pk in [
    ...
    #     from residencia.models.cursos import EstruturaCurso
    #     atingiu_max_disciplinas = False
    #     estrutura = self.residente.matriz.estrutura
    #     qtd_componentes_periodo_referencia = self.residente.matriz.componentecurricular_set.filter(periodo_letivo=self.residente.periodo_atual).count()
    #
    ...
    #             <= self.residente.get_ultima_matricula_periodo().matriculas_diarios_diario_residencia_set.filter(situacao=MatriculaDiario.SITUACAO_CURSANDO).count()
    #         ):
    #             atingiu_max_disciplinas = True
    #     return atingiu_max_disciplinas
    #
    # def utiliza_nota_atitudinal(self):
    #     return self.residente.matriz.nivel_ensino_id == NivelEnsino.MEDIO and Configuracao.get_valor_por_chave('edu', 'nota_atitudinal')


class MatriculaDiario(LogResidenciaModel):
    SITUACAO_CURSANDO = 1
    SITUACAO_APROVADO = 2
    ...
    # Fields
    matricula_periodo = models.ForeignKeyPlus('residencia.MatriculaPeriodo', related_name='matriculas_diarios_matricula_periodo_residencia_set', )
    diario = models.ForeignKeyPlus('residencia.Diario', related_name='matriculas_diarios_diario_residencia_set',)
    ...
    SEARCH_FIELDS = [
        'matricula_periodo__residente__matricula',
        'matricula_periodo__residente__pessoa_fisica__nome',
    ...
    class Meta:
        ...
        permissions = (
            ('delete_matriculadiario_cancelada_trancada', 'Pode excluir do diário residente com matrícula período cancelada ou trancada'),)
    ...
    #     return '<p>Aluno: {} </p><p>Diário: {}</p><p>Situação: {}</p>'.format(self.matricula_periodo.residente, self.diario, self.get_situacao_display())
    #
    ...
    def pode_ser_excluido_do_diario(self, user):
        pode_excluir_cancelado = self.matricula_periodo.situacao.pk in SituacaoMatriculaPeriodo.SITUACOES_INATIVAS_PARA_EXCLUSAO_DIARIO and user.has_perm('residencia.delete_matriculadiario_cancelada_trancada')
        pode_excluir_matriculado = self.matricula_periodo.situacao.pk == SituacaoMatriculaPeriodo.MATRICULADO and user.has_perm('residencia.delete_matriculadiario')

        if pode_excluir_cancelado or pode_excluir_matriculado:
            qs_periodo_posterior = MatriculaPeriodo.objects.filter(residente=self.matricula_periodo.residente, ano_letivo__ano__gt=self.matricula_periodo.ano_letivo.ano).exclude(
                situacao__id=SituacaoMatriculaPeriodo.EM_ABERTO
            )
            if self.matricula_periodo.periodo_letivo == 1:
                qs_periodo_posterior = qs_periodo_posterior | MatriculaPeriodo.objects.filter(residente=self.matricula_periodo.residente, ano_letivo__ano=self.matricula_periodo.ano_letivo.ano, periodo_letivo=2).exclude(
                    situacao__id=SituacaoMatriculaPeriodo.EM_ABERTO
                )
    ...
    #     from residencia.models import RegistroHistorico
    #
    ...
    #         registro_historico.conceito = self.matricula_periodo.residente.matriz.estrutura.get_conceito(nota)
    #     else:
    ...
    def get_notas_etapa(self, etapa):
        from residencia.models.diarios import NotaAvaliacao
    ...
    #     from residencia.models import EstagioDocente
        from residencia.models.curso import ComponenteCurricular

        pk = self.pk
        super().save(*args, **kwargs)
        if not pk:
            if self.diario.componente_curricular.tipo == ComponenteCurricular.PRATICA:
    ...
    #                 self.matricula_periodo.residente,
    #                 type(self),
    #                 self.pk,
    #                 ch_extensao,
    ...
    #             )
    #         else:
    #             AtividadeCurricularExtensao.registrar(self.matricula_periodo.residente, type(self), self.pk, 0, self.diario.componente_curricular.componente.descricao_historico, False)
    #
    # def delete(self, *args, **kwargs):
    #     from residencia.models.cursos import ComponenteCurricular
    #
    ...
    def criar_registros_notas(self):
        from residencia.models.diarios import ItemConfiguracaoAvaliacao, ConfiguracaoAvaliacao

        # etapas = list(range(1, self.diario.componente_curricular.qtd_avaliacoes + 1))
        etapas = list(range(1, 5))
        ...
        for etapa in etapas:
            if not self.notaavaliacao_set.filter(item_configuracao_avaliacao__configuracao_avaliacao__etapa=etapa).exists():
                ...
                if not configuracao_avaliacao.itemconfiguracaoavaliacao_set.exists():
                    ...
                    for matricula_diario in self.diario.matriculas_diarios_diario_residencia_set.all():
    ...
    def registrar_nota_etapa(self, etapa, considerar_nota_vazia=False):
        from residencia.models.diarios import NotaAvaliacao, ConfiguracaoAvaliacao
    ...
    #     from residencia.models import ComponenteCurricular, EstruturaCurso
    #     # plano de retomada de aulas em virtude da pandemia (COVID19)
    #     if self.matricula_periodo.residente.matriz.estrutura.tipo_avaliacao == EstruturaCurso.TIPO_AVALIACAO_MODULAR and not self.diario.turma.pertence_ao_plano_retomada:
    #         raise ValidationError('Não é possível cancelar a matrícula no diário de um residente do regime modular.')
    #
    #     if self.situacao == MatriculaDiario.SITUACAO_CANCELADO:
    #         return
    #
    #     if not self.situacao == MatriculaDiario.SITUACAO_CURSANDO:
    #         raise ValidationError('A situação do residente no diário deve ser "Cursando".')
    #
    #     if not self.matricula_periodo.residente.matriz.estrutura.tipo_avaliacao == 1:
    #         if not self.diario.turma.pertence_ao_plano_retomada:
    #             raise ValidationError('O regime de avaliação do residente deve ser "Crédito".')
    ...
    #         if not self.diario.turma.pertence_ao_plano_retomada:
    #             raise ValidationError('Matrículas em diários de residentes de primeiro período não podem ser canceladas.')
    #
    #     ultima_matricula_periodo = self.matricula_periodo.residente.get_ultima_matricula_periodo()
    #
    #     estrutura = ultima_matricula_periodo.residente.matriz.estrutura
    ...
    #         if estrutura.qtd_minima_disciplinas >= ultima_matricula_periodo.matriculas_diarios_matricula_periodo_residencia_set.filter(situacao=MatriculaDiario.SITUACAO_CURSANDO).count():
    #             optativa = self.diario.componente_curricular.optativo
    #             if optativa:
    #                 cumpre_optativas = self.matricula_periodo.residente.completou_ch_componentes_regulares_optativos()
    #                 if not cumpre_optativas:
    #                     ch_cursando = self.matricula_periodo.residente.get_ch_componentes_regulares_optativos_cursando() - self.diario.componente_curricular.componente.ch_hora_relogio
    #                     ch_pendente = self.matricula_periodo.residente.get_ch_componentes_regulares_optativos_pendente()
    ...
    #             situacao=MatriculaDiario.SITUACAO_CURSANDO, diario__componente_curricular__in=co_requisitos, matricula_periodo__residente=self.matricula_periodo.residente
    #         )
    #         if matricula.exists():
    ...
    #             falta.quantidade = 0
    #
    #         abono_faltas = self.matricula_periodo.residente.abonofaltas_set.filter(data_inicio__lte=aula.data, data_fim__gte=aula.data)
    #         if abono_faltas:
    ...
    def get_situacao_diario(self, ignorar_fechamento_carga_horaria=False, zerar_avaliacoes_nao_realizadas=False):
        ...
        # Se o período já estiver fechado para o residente, a situação é computada a partir do atributo 'situção' da classe
        ...
        # o período do residente está aberto
        if self.situacao == MatriculaDiario.SITUACAO_CURSANDO:
        ...
        # o período do residente está fechado
        else:
            if self.situacao == MatriculaDiario.SITUACAO_APROVADO:
                status = 'success'
            elif self.situacao == MatriculaDiario.SITUACAO_REPROVADO:
    ...
    def is_cursando(self):
        """
        Este método retorna verdadeiro caso o período não tenha sido fechado para o residente.
    ...
    #         matricula_periodo__residente=residente,
    ...
    #     """
    #     Este método retorna verdadeiro caso o residente tenha realizado todas as avaliações regulares e tenha sido aprovado.
    #     """
    #     if self.realizou_todas_avaliacoes_regulares():
    #         if self.get_media_disciplina() < self.matricula_periodo.residente.matriz.estrutura.media_evitar_reprovacao_direta:
    #             return True
    #     return False
    ...
    def get_situacao_nota(self):
        ...
        A situação deve ser computada conforme a seguir:
            ...
            Senão,
                Se o residente realizou todas as avaliações regulares
                    Se o residente estiver aprovado direto, a situação será 'Aprovado'
    ...
    def realizou_todas_avaliacoes(self):
        ...
        O valor da avaliação só precisa ser observado se, e somente se, o residente estiver em prova final.
        """
        if self.realizou_todas_avaliacoes_regulares():
            if self.is_em_prova_final():
    ...
    #     """
    #     Este método retorna verdadeiro caso o residente tenha alcançado a média através das avaliações regulares, ou seja, sem prova final.
    ...
    #             return True
    #         else:
    #             return self.get_media_disciplina() >= self.matricula_periodo.residente.matriz.estrutura.media_aprovacao_sem_prova_final
    #     return False
    #
    ...
    #     """
    #     Este método retorna verdadeiro se o residente realizou todas as avaliações regulares e:
    ...
    #     """
    #     Este método retorna verdadeiro se o residente tiver atingido a média após realizar a avaliação final.
    ...
    #     else:
    #         media_aprovacao_avaliacao_final = self.matricula_periodo.residente.matriz.estrutura.media_aprovacao_avaliacao_final
    ...
    #     """
    #     Este método retorna verdadeiro caso o residente tenha sido aprovado direto ou na prova final.
    #     """
    #     return self.is_aprovado_sem_prova_final() or self.is_aprovado_em_prova_final()
    #
    # def is_aprovado(self):
    #     """
    #     Este método retorna verdadeiro caso o residente tenha sido aprovado por nota e por frequência
    #     """
    ...
    #     from residencia.models.diarios import ItemConfiguracaoAvaliacao, NotaAvaliacao
    ...
    #     from residencia.models import ConfiguracaoAvaliacao
    #
    ...
    #         return self.matricula_periodo.residente.matriz.estrutura.get_conceito(nota)
    #     else:
    ...
    #         self.diario.pk,
    #         self.matricula_periodo.residente.matricula,
    ...
    #     """
    #     from residencia.models import Falta
    #
    ...
    #             diario__componente_curricular=self.diario.componente_curricular, matricula_periodo__residente=self.matricula_periodo.residente, situacao=self.SITUACAO_CANCELADO
    #         )
    ...
    #     from residencia.models import NotaAvaliacao
    #
    ...
    def get_frequencia_confirmada_total(self):
        total = 0
        for frequencia in self.matriculadiario_frequencia_residente_set.filter(confirmacao=True):
            delta = frequencia.data_hora_saida - frequencia.data_hora_entrada
...
#     equivalencia_componente = models.ForeignKeyPlus('residencia.EquivalenciaComponenteQAcademico')
...
#
#     def __str__(self):
#         return 'Registro de histórico do residente {} na disciplina {}'.format(self.matricula_periodo.residente.matricula, self.equivalencia_componente)
#
#     def delete(self, *args, **kwargs):
#         self.matricula_periodo.residente.atualizar_situacao('Reprocessamento do Histórico')
#         if perms.realizar_procedimentos_academicos(tl.get_user(), self.matricula_periodo.residente.curso_campus):
#             super().delete(*args, **kwargs)
#         else:
...
#
#     def get_periodo_matriz(self):
#         from residencia.models import ComponenteCurricular
#
#         if self.equivalencia_componente.componente:
#             qs_componente_curricular = ComponenteCurricular.objects.filter(matriz=self.matricula_periodo.residente.matriz, componente=self.equivalencia_componente.componente)
...
#
#
# class CertificacaoConhecimento(LogResidenciaModel):
#     matricula_periodo = models.ForeignKeyPlus('residencia.MatriculaPeriodo', verbose_name='Matrícula Período')
#     componente_curricular = models.ForeignKeyPlus('residencia.ComponenteCurricular', verbose_name='Componente Curricular')
#     data = models.DateFieldPlus('Data')
#     nota = models.NotaField('Nota', blank=True, null=True)
#     professores = models.ManyToManyFieldPlus('residencia.Professor', verbose_name='Professores', blank=True, help_text='Professor(es) da comissão responsável pela certificação.')
...
#
#     def adicionar_registro_historico(self, matriculado_periodo):
#         from residencia.models import RegistroHistorico, MatriculaDiario
#
#         registro_historico = RegistroHistorico()
#         registro_historico.matricula_periodo = matriculado_periodo
...
#
#     def __str__(self):
#         return 'Certificação da disciplina {} pelo residente {} '.format(self.componente_curricular.componente.sigla, self.matricula_periodo.residente.matricula)
#
#     def get_absolute_url(self):
#         return '/edu/visualizar/edu/certificacaoconhecimento/{:d}/'.format(self.pk)
#
#     def is_residente_aprovado(self):
#         return not self.ausente and self.nota >= self.matricula_periodo.residente.matriz.estrutura.media_certificacao_conhecimento
#
#     def get_situacao_certificacao(self):
#         if self.ausente:
#             return 'Ausente Cert. Conhecimento'
#         if self.is_residente_aprovado():
#             return 'Cert. Conhecimento'
#         return 'Reprovado Cert. Conhecimento'
#
#     def get_media_final_disciplina_exibicao(self):
#         if self.componente_curricular.avaliacao_por_conceito:
#             return self.matricula_periodo.residente.matriz.estrutura.get_conceito(self.nota)
#         else:
#             return mask_nota(self.nota)
#
#     def save(self, *args, **kwargs):
#         super().save(*args, **kwargs)
#         self.matricula_periodo.residente.atualizar_situacao('Lançamento de Certificação de Conhecimento')
#
#     def delete(self, *args, **kwargs):
#         super().delete(*args, **kwargs)
#         self.matricula_periodo.residente.atualizar_situacao('Exclusão de Certificação de Conhecimento')
#
#     def get_residente(self):
#         return self.matricula_periodo.residente
#
#
# class AproveitamentoEstudo(LogResidenciaModel):
#     matricula_periodo = models.ForeignKeyPlus('residencia.MatriculaPeriodo')
#     componente_curricular = models.ForeignKeyPlus('residencia.ComponenteCurricular')
#     data = models.DateFieldPlus('Data')
...
#     professores = models.ManyToManyFieldPlus('residencia.Professor', verbose_name='Professores', blank=True, help_text='Professor(es) da comissão responsável pelo aproveitamento.')
...
#
#     def __str__(self):
#         return 'Aproveitamento da disciplina {} pelo residente {}'.format(self.componente_curricular.componente.sigla, self.matricula_periodo.residente.matricula)
#
#     def get_absolute_url(self):
#         return '/edu/visualizar/edu/certificacaoconhecimento/{:d}/'.format(self.pk)
#
...
#         if self.componente_curricular.avaliacao_por_conceito:
#             nota = self.nota
#             if nota is None or nota < self.matricula_periodo.residente.matriz.estrutura.media_aprovacao_sem_prova_final:
#                 nota = self.matricula_periodo.residente.matriz.estrutura.media_aprovacao_sem_prova_final
#             return self.matricula_periodo.residente.matriz.estrutura.get_conceito(nota)
#         else:
#             return mask_nota(self.nota)
#
#     def save(self, *args, **kwargs):
#         super().save(*args, **kwargs)
#         self.matricula_periodo.residente.atualizar_situacao('Lançamento de Aproveitamento de Estudo')
#
#     def delete(self, *args, **kwargs):
#         super().delete(*args, **kwargs)
#         self.matricula_periodo.residente.atualizar_situacao('Exclusão de Aproveitamento de Estudo')
#
#
...
#
#     def __str__(self):
#         return 'Registro de histórico do residente {} na disciplina {}'.format(self.matricula_periodo.residente.matricula, self.componente.sigla)
#
#     def adicionar_registro_historico(self, matriculado_periodo):
...
class MatriculaUnidadeAprendizagemTurma(LogResidenciaModel):
    SITUACAO_CURSANDO = 1
    SITUACAO_APROVADO = 2
    SITUACAO_REPROVADO = 3
    SITUACAO_PROVA_FINAL = 4
    ...
    # Fields
    matricula_periodo = models.ForeignKeyPlus('residencia.MatriculaPeriodo', related_name='matriculas_unidadeaprendizagemturma_matricula_periodo_set', )
    unidade_aprendizagem_turma = models.ForeignKeyPlus('residencia.UnidadeAprendizagemTurma', related_name='matriculas_unidadeaprendizagemturma_unidade_aprendizagem_turma_set', null=True, on_delete=models.SET_NULL)
    ...
    SEARCH_FIELDS = [
        'matricula_periodo__residente__matricula',
        'matricula_periodo__residente__pessoa_fisica__nome',
        'unidade_aprendizagem_turma__unidade_aprendizagem__descricao',
        'unidade_aprendizagem_turma__id',
    ]
    ...
    def get_notas_etapa(self, etapa):
        from residencia.models.turmas import NotaAvaliacaoUnidadeAprendizagem

        return (
            NotaAvaliacaoUnidadeAprendizagem.objects.filter(matricula_unidade_aprendizagem_turma=self, item_configuracao_avaliacao__configuracao_avaliacao__etapa=etapa)
            .order_by('item_configuracao_avaliacao__id')
            .select_related('item_configuracao_avaliacao', 'matricula_unidade_aprendizagem_turma', 'item_configuracao_avaliacao__configuracao_avaliacao')
        )
    ...
    def criar_registros_notas(self):
        from residencia.models.turmas import ItemConfiguracaoAvaliacaoUnidadeAprendizagem, ConfiguracaoAvaliacaoUnidadeAprendizagem

        # etapas = list(range(1, self.diario.componente_curricular.qtd_avaliacoes + 1))
        etapas = list(range(1, 5))
        # etapas = etapas and etapas + [5] or []
        for etapa in etapas:
            if not self.notaavaliacaounidadeaprendizagem_set.filter(item_configuracao_avaliacao__configuracao_avaliacao__etapa=etapa).exists():
                configuracao_avaliacao, _ = ConfiguracaoAvaliacaoUnidadeAprendizagem.objects.get_or_create(unidadeaprendizagemturma=self.unidade_aprendizagem_turma, etapa=etapa)
                if not configuracao_avaliacao.itemconfiguracaoavaliacaounidadeaprendizagem_set.exists():
                    item_configuracao_avaliacao = ItemConfiguracaoAvaliacaoUnidadeAprendizagem.objects.create(configuracao_avaliacao=configuracao_avaliacao, sigla='A{}'.format(etapa))
                    for matricula_unidade_aprendizagem_turma in self.unidade_aprendizagem_turma.matriculas_unidadeaprendizagemturma_unidade_aprendizagem_turma_set.all():
                        matricula_unidade_aprendizagem_turma.criar_registros_notas_etapa(etapa, [item_configuracao_avaliacao])
                self.criar_registros_notas_etapa(etapa, configuracao_avaliacao.itemconfiguracaoavaliacaounidadeaprendizagem_set.all())

    def criar_registros_notas_etapa(self, etapa, itens_configuracao_avaliacao):
        from residencia.models.turmas import NotaAvaliacaoUnidadeAprendizagem

        for item_configuracao_avaliacao in itens_configuracao_avaliacao:
            qs = NotaAvaliacaoUnidadeAprendizagem.objects.filter(matricula_unidade_aprendizagem_turma=self, item_configuracao_avaliacao=item_configuracao_avaliacao)
            if not qs.exists():
                NotaAvaliacaoUnidadeAprendizagem.objects.create(matricula_unidade_aprendizagem_turma=self, item_configuracao_avaliacao=item_configuracao_avaliacao, nota=None)
        self.registrar_nota_etapa(etapa)

    def registrar_nota_etapa(self, etapa, considerar_nota_vazia=False):
        from residencia.models.turmas import NotaAvaliacaoUnidadeAprendizagem, ConfiguracaoAvaliacaoUnidadeAprendizagem

        # if not self.diario.componente_curricular.qtd_avaliacoes:
        #     return
        qs = NotaAvaliacaoUnidadeAprendizagem.objects.filter(matricula_unidade_aprendizagem_turma=self, item_configuracao_avaliacao__configuracao_avaliacao__etapa=etapa)
        id_configuracao_avaliacao = qs.values_list('item_configuracao_avaliacao__configuracao_avaliacao', flat=True).first()
        configuracao_avaliacao = ConfiguracaoAvaliacaoUnidadeAprendizagem.objects.filter(pk=id_configuracao_avaliacao).first()
        # se a forma de cálculo for a maior nota ou soma simples
        if configuracao_avaliacao and configuracao_avaliacao.forma_calculo in (ConfiguracaoAvaliacaoUnidadeAprendizagem.FORMA_CALCULO_MAIOR_NOTA, ConfiguracaoAvaliacaoUnidadeAprendizagem.FORMA_CALCULO_SOMA_SIMPLES):
            # pelo menos uma nota deve ter sido lançada
            notas_foram_lancadas = qs.filter(nota__isnull=False).exists()
        else:
            # senão todas as notas devem ter sido lançadas
    ...
    def get_unidadeaprendizagemturma(self):
        return self.unidade_aprendizagem_turma
    ...
    def get_media_disciplina(self):
        ...
        media_disciplina = 0
        qtd_avaliacoes = self.unidade_aprendizagem_turma.unidade_aprendizagem.qtd_avaliacoes
        ...
        if qtd_avaliacoes == 1:
            ...
            return round(self.get_nota_1() + self.get_nota_2()) /qtd_avaliacoes
    ...
    def get_nota_2_boletim(self):
        if self.unidade_aprendizagem_turma.configuracao_avaliacao_2() and self.unidade_aprendizagem_turma.configuracao_avaliacao_2().autopublicar or not self.unidade_aprendizagem_turma.posse_etapa_2:
            return mask_nota(self.nota_2)
        return None
    #
    # def get_nota_3(self):
