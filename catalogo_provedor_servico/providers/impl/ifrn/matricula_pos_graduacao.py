...
from catalogo_provedor_servico.providers.impl.ifrn.codes import ID_GOVBR_10054_MATRICULA_POS_GRADUACAO_IFRN
...
class MatriculaPosGraduacaoServiceProvider(AbstractMatriculaIFRN):
    @staticmethod
    ...
    def choices_nivel_ensino():
        niveis_ensino_to_exclude = (NivelEnsino.FUNDAMENTAL, NivelEnsino.MEDIO)
        return NivelEnsino.objects.exclude(pk__in=niveis_ensino_to_exclude)
    ...
    def get_id_servico_portal_govbr(self):
        return ID_GOVBR_10054_MATRICULA_POS_GRADUACAO_IFRN
    ...
    def _validate_dados_etapa(self, request, etapa, solicitacao):
        ...
        # Caso a etapa seja a 7, a rotina abaixo garante que cada anexo informado tenha uma descrição e vice-versa.
        if etapa.numero == 7:
            for i in range(1, 6):
                anexo_descricao_field = etapa.formulario.get_field(f'anexo_{i}_descricao')
                anexo_file_field = etapa.formulario.get_field(f'anexo_{i}_file')

                anexo_and_descricao_informados = 0
                if anexo_descricao_field['value']:
                    anexo_and_descricao_informados += 1
                if anexo_file_field['value'] or anexo_file_field['value_hash_sha512_link_id']:
                    anexo_and_descricao_informados += 1

                # Se anexo_and_descricao_informados for igual a 1, é sinal de que apenas parte da informação do anexo
                # em questão foi fornecida. Nesse caso, iremos cobrar que toda a informação seja fornecida.
                if anexo_and_descricao_informados == 1:
                    has_errors.append(f'Ao informar o <b>Anexo {i}</b>, você precisa fornecer uma descrição e fazer o upload do arquivo.')
    ...
    def _on_finish_recebimento_solicitacao(self, solicitacao):
        ...
        etapa_8 = self._get_etapa_para_edicao(cpf=solicitacao.cpf, numero_etapa=8)
        etapa_8.formulario.set_avaliacao(name='declaracao_etnia', status='OK')
        etapa_8.formulario.set_avaliacao(name='declaracao_didatica', status='OK')
        etapa_8.formulario.set_avaliacao(name='declaracao_legais', status='OK')
        etapa_8.formulario.set_avaliacao(name='declaracao_veracidade', status='OK')
        etapa_8.formulario.set_avaliacao(name='declaracao_conclusao', status='OK')
        SolicitacaoEtapa.update_or_create_from_etapa(etapa=etapa_8, solicitacao=solicitacao)
            ...
            if form_matricula.is_valid():
                try:
                    with transaction.atomic():
                        ...
                        solicitacao_etapa_7 = SolicitacaoEtapa.objects.get(solicitacao=solicitacao, numero_etapa=7)
                        etapa_7 = Etapa.load_from_json(solicitacao_etapa_7.get_dados_as_json())
                        ...
                        if solicitacao_etapa_arquivos.exists():
                            for sea in solicitacao_etapa_arquivos:
                                ...
                                aluno_arquivo.tipo_origem_cadastro = AlunoArquivo.TIPO_ORIGEM_CADASTRO_MATRICULA_ONLINE

                                aluno_arquivo.aluno = aluno
                                aluno_arquivo.arquivo_unico = sea.arquivo_unico
                                aluno_arquivo.nome_original = sea.nome_original

                                # Caso os arquivos em questão sejam da Etapa 7, vamos pegar a descrição imformada pelo
                                # usuário para poder exibir uma informação amigável na sua Pasta Documental e também
                                # para nomear o arquivo.
                                # Ex: Exibir "Termo de Compromisso do Candidato" ao invés de "Anexo 1".
                                if sea.solicitacao_etapa.numero_etapa == 7:
                                    anexo_extensao = '.{}'.format(sea.nome_original.split('.')[-1])
                                    anexo_numero = int(sea.nome_atributo_json.replace('anexo_', '').replace('_file', ''))
                                    anexo_descricao = etapa_7.formulario.get_value(f'anexo_{anexo_numero}_descricao')

                                    # No caso do nome de exibição do arquivo o seu limite é 50, contando com a extensão
                                    # do arquivo.
                                    # Deixando o nome no padrão "Camel Case", apenas letras e números e sem espaços. Obs:
                                    # Fiz isso pensando em fechar possível brecha que permitisse a execução de um comando
                                    # malicioso durante a criação do arquivo em disco.
                                    # Ex: A descrição do arquivo, fornercida pelo usuário, ser algo do tipo "rm /bin".
                                    nome_exibicao_ajustado = re.sub('[\\W_]+', '', anexo_descricao.title())
                                    # Limitando o nome a 50 caracteres no total, contando a extensão.
                                    nome_exibicao_ajustado = nome_exibicao_ajustado[:50 - len(anexo_extensao)]
                                    aluno_arquivo.nome_exibicao = '{}{}'.format(nome_exibicao_ajustado, anexo_extensao)

                                    # Deixando o nome no padrão "Camel Case", apenas letras e números e espaços simples.
                                    aluno_arquivo.descricao = re.sub('[\\W_]+', ' ', anexo_descricao.title())
                                else:
                                    aluno_arquivo.nome_exibicao = sea.nome_exibicao
                                    aluno_arquivo.descricao = sea.descricao

                                aluno_arquivo.data_hora_upload = sea.data_hora_upload
                                aluno_arquivo.save()
                                solicitacao.add_dado_registro_execucao(model_object=aluno_arquivo, operation='CREATE')
    ...
    def _get_next_etapa(self, cpf, etapa):
        ...
        if etapa.numero == 7:
            return self._get_etapa_8(cpf=cpf)
    ...
    def _do_avaliacao_disponibilidade_especifica(self, cpf, servico, avaliacao):
        ...
        # 3 - O aluno não poderá ter outro curso de pós-graduação da mesma modalidade ativo ao mesmo tempo

        alunos_trancados = Aluno.objects.filter(pessoa_fisica__cpf=cpf,
                                                curso_campus__modalidade__nivel_ensino=NivelEnsino.POS_GRADUACAO,
                                                situacao__in=[SituacaoMatricula.TRANCADO,
                                                              SituacaoMatricula.TRANCADO_VOLUNTARIAMENTE])
        avaliacao.add_criterio(not alunos_trancados.exists(),
                               'Não existe nenhum curso  de pós-graduação  com matrícula trancada para este CPF',
                               'Existe(m) curso(s)  de pós-graduação com matrícula trancada para este CPF')
        ...
        candidato_vagas = CandidatoVaga.matricula_online_disponibilidade(cpf=cpf, niveis_ensino=[NivelEnsino.POS_GRADUACAO])

        em_periodo_matricula = False
        em_periodo_avaliacao = False

        lista_modalidades_candidato_vagas = list()
        for candidato_vaga in candidato_vagas:
            ...
            # Adiciona a modalidade na lista de modalidades para filtro da verificação da quantidade de alunos ativos por modalidade
            lista_modalidades_candidato_vagas.append(candidato_vaga['modalidade_id'])
        ...
        # No caso do nível pois graduação pode ter duas matrículas ativas nesse nível sendo uma latosensu e outa strictusensu
        alunos_ativos_mesma_modalidade = Aluno.ativos.filter(pessoa_fisica__cpf=cpf,
                                                             curso_campus__modalidade__nivel_ensino=NivelEnsino.POS_GRADUACAO,
                                                             curso_campus__modalidade__pk__in=lista_modalidades_candidato_vagas)
        avaliacao.add_criterio(not alunos_ativos_mesma_modalidade.exists(),
                               'Não existe nenhum curso de pós-graduação ativo para este CPF.',
                               'Existem um curso de pós-graduação ativo para este CPF.')
    ...
    def _get_etapa_1(self, cpf):
        ...
        etapa_1.formulario.add_choices(label='Edital', name='candidato_vaga', value=None, choices=self.choices_candidatos_vaga, filters={'cpf': cpf, 'niveis_ensino': [NivelEnsino.GRADUACAO, NivelEnsino.POS_GRADUACAO]})
    ...
    def _get_etapa_3(self, cpf):
        ...
        if etapa_3:
            ...
            etapa_3.formulario\
                .add_boolean(label='Dados de Renda da Família Obrigatórios?', help_text=label_duvida, name='dados_renda_obrigatorio', value=bool(qtd_membros_familia), required=False, read_only=True)
            etapa_3.fieldsets.add(name='Dados de Renda da Família Obrigatórios', fields=('dados_renda_obrigatorio',))
            ...
            for i in range(qtd_arquivos_existentes, qtd_membros_familia):
                ...
                if i == 0:  # se for o próprio aluno já preenche o nome e o booleano de idade
                    etapa_3.formulario \
                        ...
                        .add_choices(label=f'Parentesco do {i + 1}º Familiar', name=f'parentesco_{field}',
                                     choices=self.choices_parentesco, value='', required=False, read_only=True) \
                    ...
                    etapa_3.formulario\
                        ...
                        .add_choices(label=f'Parentesco do {i + 1}º Familiar', name=f'parentesco_{field}',
                                     choices=self.choices_parentesco, value='', required=True) \
                        .add_file(label=f'Documento Comprovatório do {i+1}º Familiar', name=f'documento_renda_{field}', value=None, label_to_file=f'Documento de Renda do {i+1}o. Familiar', help_text=help_text, required=True)\
    ...
    def _get_etapa_5(self, cpf):
        ...
        if not etapa_5:
            ...
            etapa_5.formulario\
                ...
                .add_file(label='Cópia da Carteira de Vacinação', name='copia_carteira_vacinacao', value=None, label_to_file='Carteira de Vacinação', required=False)
            ...
            etapa_5.formulario\
                .add_file(label='Diploma de Curso Superior de Graduação', name='copia_doc_conclusao_ensino', value=None, label_to_file='Diploma de Curso Superior de Graduação', required=True) \
                .add_file(label='Histórico Acadêmico de Curso Superior de Graduação', name='copia_hist_conclusao_ensino', value=None, label_to_file='Hist. Acadêmico Curso Superior Graduação', required=True)
            etapa_5.fieldsets.add(name='Documentos Comprobatórios de Escolaridade', fields=('copia_doc_conclusao_ensino', 'copia_hist_conclusao_ensino'))

            # etapa_5.formulario \
            #     .add_file(label='Termo de Responsabilidade de Disponibilidade de Tempo',
            #               name='termo_responsabilidade_disponibilidade_tempo',
            #               value=None,
            #               label_to_file='Termo de Responsabilidade de Disponibilidade de Tempo',
            #               required=True)
            #
            # # TODO: Como saber de quem cobrar? Essa informação pode ser obtida do Processo Seletivo ou SGC?
            # exigir_campo_declaracao_escola_vinculo_area_formacao_curso = True
            # if exigir_campo_declaracao_escola_vinculo_area_formacao_curso:
            #     etapa_5.formulario\
            #         .add_file(label='Declaração da Escola de Vínculo nas Áreas de Formação do Curso',
            #                   name='declaracao_escola_vinculo_area_formacao_curso',
            #                   value=None,
            #                   label_to_file='Declaração da Escola de Vínculo nas Áreas de Formação do Curso',
            #                   required=True,
            #                   help_text='(Enquadrados como Professores de Escola Pública)')
            # if exigir_campo_declaracao_escola_vinculo_area_formacao_curso:
            #     etapa_5.fieldsets.add(name='Declarações & Termos', fields=('termo_responsabilidade_disponibilidade_tempo', 'declaracao_escola_vinculo_area_formacao_curso'))
            # else:
            #     etapa_5.fieldsets.add(name='Declarações & Termos', fields=('termo_responsabilidade_disponibilidade_tempo',))
    ...
    def _get_etapa_7(self, cpf):
        ...
        etapa_1 = self._get_etapa_para_edicao(cpf=cpf, numero_etapa=1)
        candidato_vaga_display_value = etapa_1.formulario.get_display_value('candidato_vaga')
        pergunta_01 = (
            f'Estou ciente de que, caso o edital <strong>{candidato_vaga_display_value}</strong> exija outros documentos, eu devo fornecer tais documentos aqui neste etapa.'
        )
        etapa_7.formulario\
            .add_boolean(label='Confirmo', help_text=mark_safe(pergunta_01), name='declaracao_legais', value=None, required=True)
        etapa_7.fieldsets.add(name='Declaração de ciência', fields=('declaracao_legais',))

        # Construindo a área para o upload dos documetos que variam entre os editais de pós graduação.
        for i in range(1, 6):
            etapa_7.formulario \
                .add_string(label=f'Descrição {i}', name=f'anexo_{i}_descricao', value=None, required=False, max_length=100) \
                .add_file(label=f'Anexo {i}', name=f'anexo_{i}_file', value=None, label_to_file=f'Anexo{i}', allowed_extensions=['pdf'], required=False)
            etapa_7.fieldsets.add(name=f'Anexo {i}', fields=(f'anexo_{i}_descricao', f'anexo_{i}_file',))

        return etapa_7

    # A etepa 8 deste código é exatamento igual a etapa 7 do código feito por Hugo no MatriculaSuperiorServiceProvider.
    def _get_etapa_8(self, cpf):
        etapa_8 = self._get_etapa_para_edicao(cpf=cpf, numero_etapa=8)
        ...
        if not etapa_8:
            ...
            # Configura os fields
            etapa_8 = Etapa(numero=8, total_etapas=self.get_numero_total_etapas(), nome='Etapa 8')
            ...
            if idade < 18:
                ...
                etapa_8.fieldsets.add(name='Responsabilidade por Menores de Idade', fields=('termo_responsabilidade_menor', 'documento_responsável'))
