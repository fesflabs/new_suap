    ...
    */
    ...
    function tipo_processo_onchange_callback() {
        ...
        if (tipo_processo_id_selected) {
            log_info("Carregando os níveis de acesso e classificações para o tipo_processo '{0}' (id: {1}) ...".f(
            ...
            $.get("/processo_eletronico/tipo_processo_classificacoes_nivel_acesso_padrao/" + tipo_processo_id_selected + "/", function (data) {
                ...
                if (nivel_acesso_id_onready && tipo_processo_id_onready==tipo_processo_id_selected) {
                    log_info("Nível de acesso setado: o que está atualmente cadastrado no processo ...", 3);
                    $("#id_nivel_acesso").val(nivel_acesso_id_onready);
                } else {
                    log_info("Nível de acesso setado: o padrão definido pro tipo_processo de processo selecionado ...", 3);
                    //alert(data.nivel_acesso_padrao.id);
                    $("#id_nivel_acesso").val(data.nivel_acesso_padrao.id);
                }
    ...
    $("#id_nivel_acesso").on('change', function() {
        ...
        if (tipo_processo_id_selected && nivel_acesso_id_selected) {
            ...
            $.get("/processo_eletronico/get_hipoteses_legais_by_processo_nivel_acesso/" + nivel_acesso_id_selected + "/", function (data) {
                ...
                $.each(data.hipoteses_legais, function(i, value) {
                    ...
                    if (hipotese_legal_id_onready && hipotese_legal_id_onready == value.id) {
                        log_info("Hipótese legal setada: o que está atualmente cadastrado no processo - '{0}' (id: {1}) ...".f(value.nome, value.id), 2);
