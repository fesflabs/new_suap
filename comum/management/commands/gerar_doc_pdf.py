...
# flake8: noqa
"""
Comando que imprime o código-fonte do suap no formato PDF
"""
        ...
        while not atEOF:
            ...
            while column <= self._columns:
                ...
                if column < self._columns:
                    buf = "".join(("1 0 0 1 ", str((self._pageWd / 2 + 25)), " ", str(self._pageHt - 40), " Tm\n"))
                    ws(buf)
...
class Command(BaseCommandPlus):
    def is_valid(self, file):
        if os.path.isdir(file):
            return False
        if ('__init__.py' in file) or ('djtools' in file) or ('django_evolution' in file) or ('django_extensions' in file):
            return False
    ...
    def append_list(self, file):
        if (not '.svn' in file) and os.path.isdir(file):
            for child in os.listdir(file):
                self.append_list(file + '/' + child)
        if self.is_valid(file):
            OUT.write(file)
            OUT.write('\r\n')

    def append_src(self, file):
        if (not '.svn' in file) and os.path.isdir(file):
            for child in os.listdir(file):
                self.append_src(file + '/' + child)
        if self.is_valid(file):
            tmp = open(file, 'r')
            OUT.write('--------------------------------------------------------------------------------\r\n')
            OUT.write(file)
    ...
    def handle(self, *args, **options):
        OUT.write('--------------------------------------------------------------------------------\r\n')
        OUT.write('                                   LISTA DE ARQUIVOS')
        OUT.write('\r\n--------------------------------------------------------------------------------\r\n')
        for file in os.listdir(settings.BASE_DIR):
            self.append_list(file)
        for file in os.listdir(settings.BASE_DIR):
            self.append_src(file)
        OUT.close()
        pdfclass = pyText2Pdf()
        pdfclass.parseArgs()
        pdfclass.Convert()
