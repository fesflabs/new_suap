...
from catalogo_provedor_servico.providers.impl.ifrn.codes import ID_GOVBR_6424_MATRICULA_CURSO_NIVEL_SUPERIOR_IFRN
...
class MatriculaSuperiorServiceProvider(AbstractMatriculaIFRN):
    @staticmethod
    ...
    def get_id_servico_portal_govbr(self):
        return ID_GOVBR_6424_MATRICULA_CURSO_NIVEL_SUPERIOR_IFRN
            ...
            if form_matricula.is_valid():
                try:
                    with transaction.atomic():
                        ...
                        # Envia e-mail para aluno com o comprovante de matrícula
    ...
    def _do_avaliacao_disponibilidade_especifica(self, cpf, servico, avaliacao):
        ...
        alunos_ativos_nivel_ensino = Aluno.ativos.filter(pessoa_fisica__cpf=cpf, curso_campus__modalidade__nivel_ensino=NivelEnsino.GRADUACAO)
        avaliacao.add_criterio(not alunos_ativos_nivel_ensino.exists(), 'Não existe nenhum curso de graduação ativo para este CPF', 'Existem um curso de graduação ativo para este CPF')

        alunos_trancados = Aluno.objects.filter(pessoa_fisica__cpf=cpf, curso_campus__modalidade__nivel_ensino=NivelEnsino.GRADUACAO, situacao__in=[SituacaoMatricula.TRANCADO, SituacaoMatricula.TRANCADO_VOLUNTARIAMENTE])
        avaliacao.add_criterio(not alunos_trancados.exists(), 'Não existe nenhum curso de graduação com matrícula trancada para este CPF', 'Existe(m) curso(s) de graduação com matrícula trancada para este CPF')
        ...
        candidato_vagas = CandidatoVaga.matricula_online_disponibilidade(cpf=cpf, niveis_ensino=[NivelEnsino.GRADUACAO])
        ...
        for candidato_vaga in candidato_vagas:
            ...
            try:
                for solicitacao in solicitacoes:
                    ...
                    candidato_vaga_anterior = \
                        solicitacao.solicitacaoetapa_set.get(numero_etapa=1).get_dados_as_json()['formulario'][0]['value']
